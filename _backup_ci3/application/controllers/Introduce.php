...
class Introduce extends CI_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model(['MIntroduce', 'MCategory']);
        $this->load->database();
    ...
    public function form($id = 0) {
        ...
        $data['content'] = '/admin/pages/introduces/form';
        $data['id'] = $id;
        $introduces = $this->input->post("dataForm");
        $dataError = [];
        if(!empty($introduces)) {
            if(empty($introduces['title'])) {
                $dataError['title'] = 'Tiêu đề không được để trống';
            } else {
                $checkExist = $this->MIntroduce->getOneBy(['title' => $introduces['title']]);
                if($checkExist && $checkExist->id != $id) {
                    $dataError['title'] = 'Tiêu đề đã tồn tại';
                }
            }
            $introduces['slug'] = createSlug($introduces['title']);
            ...
            if(empty($dataError)) {
                if($id == 0) {
                    $introduces['created_at'] = date('Y-m-d H:i:s');
                }
                $introduces['updated_at'] = date('Y-m-d H:i:s');
                try {
                    if($id > 0) {
                        $this->MIntroduce->update($id, $introduces);
                    } else {
                        $this->MIntroduce->insert($introduces);
                    }
                    redirect('/admin/introduces');
                } catch(Exception $ex) {
                   echo $ex;
                   die;
                }
        ...
        if(!empty($dataError)) {
            ...
            if($id > 0) {
                ...
                $introduces = $this->MIntroduce->getOneByID($id);
                $data["dataForm"] = $introduces;
            } 
        }
    ...
    public function index() {
        // is_admin();
        $data['title'] = "Danh sách chuyên gia";
       
        $total = $this->MIntroduce->count_all(); // tổng số mục
        ...
        $introduces = $this->MIntroduce->get(['limit' => $perPage, 'offset' => $offset]);
        ...
        $data['page_name'] = 'introduces';
        $data['datas'] = $introduces;
        ...
        $data['content'] = '/admin/pages/introduces/index';
        $this->load->view('admin/layouts/app', $data);
    }
