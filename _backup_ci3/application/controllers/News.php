...
class News extends CI_Controller
{
    public function __construct() {
        parent::__construct();
        $this->load->model(['MNew', 'MCategory']);
        $this->load->database();
    ...
    public function form($id = 0) {
        ...
        $data['content'] = '/admin/pages/news/form';
        $data['id'] = $id;
        $new = $this->input->post("dataForm");
        $dataError = [];
        if(!empty($new)) {
            $new['slug'] = createSlug($new['title']);
            if(empty($new['title'])) {
                $dataError['title'] = 'Tiêu đề không được để trống';
            } else {
                $checkExist = $this->MNew->getOneBy(['title' => $new['title']]);
            ...
            if(empty($dataError)) {
                ...
                try {
                    if($id > 0) {
                        $this->MNew->update($id, $new);
                    } else {
                        $this->MNew->insert($new);
                    }
                    redirect('/admin/new');
                } catch(Exception $ex) {
                   echo $ex;
    ...
    public function index() {
        ...
        $data['content'] = '/admin/pages/news/index';
        $this->load->view('admin/layouts/app', $data);
    }
